...
def get_top_n_images(
        score_file_path: str, 
        no_img: int = 10,
        restrict_list: list = None,
    ):
    data = pd.read_csv(score_file_path)

    if restrict_list:
        # get basename of data['image']
        data['image_basename'] = data['img'].apply(lambda x: os.path.basename(x))
        restrict_df = pd.DataFrame(restrict_list, columns=['img'])
        # remove restricted images from data
        data = data[~data['image_basename'].isin(restrict_df['img'])]
        data.drop('image_basename', axis=1, inplace=True)
        # get top n images
        image_list = data.iloc[:no_img, 2]
    else:
        data['img'] = data['img'].apply(lambda x: os.path.basename(x))
        image_list = data.iloc[:no_img, 2]
    
    return image_list.values.tolist()

def get_bottom_n_images(
        score_file_path: str, 
        no_img: int = 10,
        restrict_list: list = None,
    ):
    ...
    if restrict_list:
        ...
        data['image_basename'] = data['img'].apply(lambda x: os.path.basename(x))
        restrict_df = pd.DataFrame(restrict_list, columns=['img'])
        # remove images with no instances
        data = data[data['no of predictions'] > 0]
        # remove restricted images from data
        data = data[~data['image_basename'].isin(restrict_df['img'])]
        data.drop('image_basename', axis=1, inplace=True)
        # get bottom n images
